...
def send_query(connection, query_str):
...
def get_raw_data(accounting_date):
    query_str = f"""
        SELECT 
            CD.CD_TEXT,
            CM.CM_NAME,
            ...
            FI_DEBTOR_CM_SEQ,
            FI_ACCOUNT
        FROM FINANCE_IMPORT AS FI
        JOIN CHRIST_MEMBER AS CM
        ON FI.FI_DEBTOR_CM_SEQ = CM.CM_SEQ AND FI_IN_DATE='{accounting_date}'
    ...
    query_result = send_query(con, query_str)
